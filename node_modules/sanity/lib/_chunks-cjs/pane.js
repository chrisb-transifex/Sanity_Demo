...
function PaneItemPreview(props) {
  const { icon, layout, presence, schemaType, value } = props, title = sanity.isRecord(value.title) && React.isValidElement(value.title) || isString__default.default(value.title) || isNumber__default.default(value.title) ? value.title : null, previewStateObservable = React.useMemo(
  ...
  ), { draft, published, isLoading } = reactRx.useObservable(previewStateObservable, {
    draft: null,
    isLoading: !0,
    published: null
...
`, Pane = React.forwardRef(function(props, forwardedRef) {
  ...
  const {
    ...
    mount,
    panes
  } = usePaneLayout(), pane2 = panes.find((p) => p.element === rootElement), paneIndex = pane2 && panes.indexOf(pane2), nextPane = typeof paneIndex == "number" ? panes[paneIndex + 1] : void 0, isLast = paneIndex === panes.length - 1, collapsed = layoutCollapsed ? !1 : (pane2 == null ? void 0 : pane2.collapsed) || !1, nextCollapsed = (nextPane == null ? void 0 : nextPane.collapsed) || !1, ref = React.useRef(null), flex = (_a = pane2 == null ? void 0 : pane2.flex) != null ? _a : flexProp, currentMinWidth = (_b = pane2 == null ? void 0 : pane2.currentMinWidth) != null ? _b : currentMinWidthProp, currentMaxWidth = (_c = pane2 == null ? void 0 : pane2.currentMaxWidth) != null ? _c : currentMaxWidthProp;
...
function DocumentInspectorMenuItemsResolver(props) {
  ...
  const InspectorMenuItems = React.useMemo(() => inspectors2.map((inspector, index) => inspector.useMenuItem ? [
...
function defineInspectorMenuItemComponent({
  ...
  return React.memo(function() {
    const menuItem = useMenuItem2({
      documentId,
      documentType
    }), node = sanity.useUnique(menuItem);
    return React.useEffect(() => {
...
const ReferenceChangedBanner = React.memo(() => {
  ...
  }, [documentPreviewStore, parentId, parentRefPath]), referenceInfo = reactRx.useObservable(referenceInfoObservable, { loading: !0 }), handleReloadReference = React.useCallback(() => {
    var _a2;
...
const preventDefault = (ev) => ev.preventDefault(), FormView = React.forwardRef(function(props, ref) {
  ...
  React.useEffect(() => {
    !focusedFirstDescendant && ready && !(formState != null && formState.focusPath.length) && formRef && (setFocusedFirstDescendant(!0), ui.focusFirstDescendant(formRef));
...
function DocumentStatusBarActionsInner(props) {
  var _a, _b, _c;
  const { disabled, showMenu, states } = props, { __internal_tasks, schemaType, openPath } = useDocumentPane(), [firstActionState, ...menuActionStates] = states, [buttonElement, setButtonElement] = React.useState(null), isTreeArrayEditingEnabled = (_c = (_b = (_a = sanity.useSource().features) == null ? void 0 : _a.beta) == null ? void 0 : _b.treeArrayEditing) == null ? void 0 : _c.enabled, isTreeArrayEditingEnabledOpen = React.useMemo(() => isTreeArrayEditingEnabled ? sanity.shouldArrayDialogOpen(schemaType, openPath) : !1, [isTreeArrayEditingEnabled, openPath, schemaType]), tooltipContent = React.useMemo(() => !firstActionState || !firstActionState.title && !firstActionState.shortcut ? null : /* @__PURE__ */ jsxRuntime.jsxs(ui.Flex, { style: { maxWidth: 300 }, align: "center", gap: 3, children: [
...
const DocumentStatusBarActions = React.memo(function() {
  const { actions: allActions, connectionState, documentId, editState } = useDocumentPane(), actions = React.useMemo(
    () => (allActions != null ? allActions : []).filter((action) => !isRestoreAction(action)),
    [allActions]
  );
  return actions.length === 0 || !editState ? null : /* @__PURE__ */ jsxRuntime.jsx(
...
const DocumentPanelHeader = React.memo(
  React.forwardRef(function(_props, ref) {
    ...
    } = useDocumentPane(), { features } = useStructureTool(), { index, BackLink: BackLink2, hasGroupSiblings } = usePaneRouter(), { actions: fieldActions } = sanity.useFieldActions(), [referenceElement, setReferenceElement] = React.useState(null), actions = React.useMemo(
      () => (allActions != null ? allActions : []).filter((action) => !isRestoreAction(action)),
      [allActions]
    ), menuNodes = React.useMemo(
...
function usePreviewUrl(value) {
  const { resolveProductionUrl } = sanity.useSource().document, subject = React.useMemo(
    () => new rxjs.BehaviorSubject(void 0),
    []
  );
  React.useEffect(() => {
    subject.next(value);
  }, [subject, value]);
  const resolvedUrlObservable = React.useMemo(() => subject.asObservable().pipe(
  ...
  return reactRx.useObservable(resolvedUrlObservable);
}
  ...
  return rxjs.merge(
    ...
    rxjsExhaustmapWithTrailing.exhaustMapWithTrailing((event) => (staticTypeNames ? rxjs.of(staticTypeNames) : client.observable.fetch(`array::unique(*[${filter}][]._type)`, params)).pipe(
      rxjs.mergeMap((typeNames) => {
        const types2 = sanity.getSearchableTypes(schema, staticTypeNames || []).filter((type) => typeNames.includes(type.name) ? (getExtendedProjection(type, sort.by, !0), !0) : !1), search = sanity.createSearch(types2, client, {
          filter,
          params,
          enableLegacySearch,
...
`, DocumentListPane = React.memo(function(props) {
  const { childItemId, isActive, pane: pane2, paneKey, sortOrder: sortOrderRaw, layout } = props, schema = sanity.useSchema(), { displayOptions, options } = pane2, { apiVersion, filter } = options, params = useShallowUnique(options.params || EMPTY_RECORD), typeName = React.useMemo(() => {
    ...
    sortOrder
  }), handleQueryChange = reactRx.useObservableEvent(
    (event$) => event$.pipe(
...
function DocumentSheetListSelect(props) {
  const { row, table } = props, { selectedAnchor, setSelectedAnchor } = table.options.meta || {}, handleOnClick = React.useCallback(
    (e) => {
...
function DocumentSheetListProvider({
  ...
  const [selectedAnchorCellDetails, setSelectedAnchorCellDetails] = React.useState(null), [selectedRangeCellIndexes, setSelectedRangeCellIndexes] = React.useState([]), clearAndSetFocusSelection = React.useCallback(
  ...
  ), resetFocusSelection = React.useCallback(
    () => clearAndSetFocusSelection(),
    [clearAndSetFocusSelection]
  ), changeSelectionColumn = React.useCallback(
    (direction) => {
  ...
  ), changeSelectionRange = React.useCallback(
    (direction) => {
    ...
    [selectedAnchorCellDetails]
  ), setSelectedAnchorCell = React.useCallback(
    (colId, rowIndex) => {
      clearAndSetFocusSelection({ colId, rowIndex, state: "selected" });
    },
    [clearAndSetFocusSelection]
  ), handleEscapePress = React.useCallback(() => {
  ...
  }, [clearAndSetFocusSelection, selectedAnchorCellDetails, selectedRangeCellIndexes.length]), handleUpDownKey = React.useCallback(
    (isShiftKey, key) => {
      if (!selectedAnchorCellDetails) return;
  ...
  ), handleAnchorKeydown = React.useCallback(
    (event) => {
      if (!selectedAnchorCellDetails) return;
  ...
  ), handleAnchorClick = React.useCallback(
    (event) => {
      var _a;
  ...
  ), submitFocusedCell = React.useCallback(() => {
...
function SheetListCellInner(props) {
  const { getValue, column, row, fieldType } = props, cellId = `cell-${column.id}-${row.index}`, [renderValue, setRenderValue] = React.useState(getValue()), [isDirty, setIsDirty] = React.useState(!1), inputRef = React.useRef(null), {
    focusAnchorCell,
    resetFocusSelection,
    setSelectedAnchorCell,
    getStateByCellId,
  ...
  }, handleOnEnterDown = React.useCallback(
    (event) => {
      const { key } = event;
  ...
  return fieldType.name === "boolean" ? /* @__PURE__ */ jsxRuntime.jsxs(
    ui.Select,
    {
      ...inputProps,
      onChange: () => null,
...
function SheetListCell(cell) {
  ...
  return /* @__PURE__ */ jsxRuntime.jsx(
    Cell,
    {
      ...
      children: reactTable.flexRender(cell.column.columnDef.cell, (_b = cell.getContext) == null ? void 0 : _b.call(cell))
    },
...
const VISIBLE_COLUMN_LIMIT = 5, PreviewCell = (props) => {
  var _a, _b;
  const { documentPreviewStore, row, schemaType } = props, title = "Document title", previewStateObservable = React.useMemo(
...
function ColumnsControl({ table }) {
  const { t } = sanity.useTranslation(), isVisibleLimitReached = table.getVisibleLeafColumns().filter((col) => col.getCanHide()).length >= VISIBLE_COLUMN_LIMIT, setInitialColumns = React.useCallback(() => {
  ...
  return /* @__PURE__ */ jsxRuntime.jsx(
    ui.MenuButton,
    {
      button: /* @__PURE__ */ jsxRuntime.jsx(ui.Button, { mode: "bleed", text: t("sheet-list.edit-columns"), size: 1 }),
      id: "columns-control",
      menu: /* @__PURE__ */ jsxRuntime.jsx(ui.Menu, { padding: 3, paddingTop: 4, style: { width: 240 }, children: /* @__PURE__ */ jsxRuntime.jsxs(ui.Flex, { direction: "column", height: "fill", gap: 3, children: [
        /* @__PURE__ */ jsxRuntime.jsx(ui.Text, { weight: "semibold", size: 1, children: t("sheet-list.select-fields") }),
        /* @__PURE__ */ jsxRuntime.jsx(ui.Flex, { style: { flex: "1 1 auto", maxHeight: 320, overflowY: "scroll" }, children: /* @__PURE__ */ jsxRuntime.jsx(ui.Stack, { children: table.getAllLeafColumns().filter((column) => column.getCanHide()).map((column) => {
          var _a;
...
function DocumentSheetListHeader(props) {
  ...
  return /* @__PURE__ */ jsxRuntime.jsx(
    HeaderTag,
    {
      ...
      children: /* @__PURE__ */ jsxRuntime.jsxs(ui.Flex, { justify: "space-between", marginX: 2, align: "baseline", children: [
        /* @__PURE__ */ jsxRuntime.jsx(TooltipDelayGroupProvider.Tooltip, { delay: 500, content: headerTitle, children: /* @__PURE__ */ jsxRuntime.jsx(ui.Box, { style: { boxSizing: "border-box" }, marginLeft: 3, marginRight: 3, children: headerTitle }) }),
        canShowHeaderMenu && /* @__PURE__ */ jsxRuntime.jsx(HoverMenu, { children: /* @__PURE__ */ jsxRuntime.jsx(
          ui.MenuButton,
          {
            button: /* @__PURE__ */ jsxRuntime.jsx(
              TooltipDelayGroupProvider.Button,
...
function DocumentSheetListPaneInner({
  ...
  const renderRow = React.useCallback((row) => /* @__PURE__ */ jsxRuntime.jsx(
    ui.Box,
    {
      ...
      children: row.getVisibleCells().map((cell) => /* @__PURE__ */ React.createElement(SheetListCell, { ...cell, key: row.original._id + cell.id }))
    },
  ...
  return /* @__PURE__ */ jsxRuntime.jsxs(PaneContainer$1, { direction: "column", paddingX: 3, "data-testid": "document-sheet-list-pane", children: [
    ...
    /* @__PURE__ */ jsxRuntime.jsx(TableContainer, { children: /* @__PURE__ */ jsxRuntime.jsx(DocumentSheetListProvider, { table, children: /* @__PURE__ */ jsxRuntime.jsxs(Table, { children: [
      /* @__PURE__ */ jsxRuntime.jsx("thead", { children: table.getHeaderGroups().map((headerGroup) => /* @__PURE__ */ jsxRuntime.jsx(ui.Box, { as: "tr", children: headerGroup.headers.map((header) => /* @__PURE__ */ jsxRuntime.jsx(
