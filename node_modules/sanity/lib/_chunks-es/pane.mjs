import { Tooltip, TooltipDelayGroupProvider, Button, Dialog, MenuGroup, MenuItem, MenuButton, Tab, Popover as Popover$1, generateHelpUrl } from "./generate-help-url.esm.mjs";
import { jsx, Fragment, jsxs } from "react/jsx-runtime";
import { Box, rem, Flex, Inline, Text, useToast, Card, Stack, ErrorBoundary, Layer, BoundaryElementProvider, MenuDivider, Menu, TextSkeleton, rgba, useElementRect, LayerProvider, _raf2, TextInput, TabList, TabPanel, Code, Heading, focusFirstDescendant, Container, Spinner, usePortal, PortalProvider, Popover, useLayer, useClickOutside, useGlobalKeyDown, Grid, Button as Button$1, Dialog as Dialog$1, Hotkeys, AvatarStack, Badge, DialogProvider, Checkbox, Select, MenuButton as MenuButton$1 } from "@sanity/ui";
import { isValidElement, useMemo, forwardRef, useContext, useCallback, useId, useState, useEffect, useRef, useImperativeHandle, memo, useLayoutEffect, createElement, useReducer, Fragment as Fragment$1, lazy } from "react";
...
import { fromEvent, timer, EMPTY, of, merge, concat, firstValueFrom, filter, BehaviorSubject, from, defer, mergeMap, throwError, share, partition, take, throttleTime, asyncScheduler, map as map$1, Subject, tap as tap$1, debounce, catchError as catchError$1 } from "rxjs";
...
const structureLocaleNamespace = "structure", structureUsEnglishLocaleBundle = defineLocaleResourceBundle({
  ...
  resources: () => import("./resources3.mjs")
});
...
function PaneItemPreview(props) {
  const { icon, layout, presence, schemaType, value } = props, title = isRecord(value.title) && isValidElement(value.title) || isString(value.title) || isNumber(value.title) ? value.title : null, previewStateObservable = useMemo(
    () => getPreviewStateObservable(props.documentPreviewStore, schemaType, value._id, title),
    [props.documentPreviewStore, schemaType, title, value._id]
  ), { draft, published, isLoading } = useObservable(previewStateObservable, {
    draft: null,
    isLoading: !0,
    published: null
...
`, Pane = forwardRef(function(props, forwardedRef) {
  ...
  const {
    ...
    mount,
    panes
  } = usePaneLayout(), pane2 = panes.find((p) => p.element === rootElement), paneIndex = pane2 && panes.indexOf(pane2), nextPane = typeof paneIndex == "number" ? panes[paneIndex + 1] : void 0, isLast = paneIndex === panes.length - 1, collapsed = layoutCollapsed ? !1 : (pane2 == null ? void 0 : pane2.collapsed) || !1, nextCollapsed = (nextPane == null ? void 0 : nextPane.collapsed) || !1, ref = useRef(null), flex = (_a = pane2 == null ? void 0 : pane2.flex) != null ? _a : flexProp, currentMinWidth = (_b = pane2 == null ? void 0 : pane2.currentMinWidth) != null ? _b : currentMinWidthProp, currentMaxWidth = (_c = pane2 == null ? void 0 : pane2.currentMaxWidth) != null ? _c : currentMaxWidthProp;
...
function DocumentInspectorMenuItemsResolver(props) {
  ...
  const InspectorMenuItems = useMemo(() => inspectors2.map((inspector, index) => inspector.useMenuItem ? [
...
const ReferenceChangedBanner = memo(() => {
  ...
  const documentPreviewStore = useDocumentPreviewStore(), { params, groupIndex, routerPanesState, replaceCurrent, BackLink: BackLink2 } = usePaneRouter(), routerReferenceId = (_a = routerPanesState[groupIndex]) == null ? void 0 : _a[0].id, parentGroup = routerPanesState[groupIndex - 1], parentSibling = parentGroup == null ? void 0 : parentGroup[0], parentId = parentSibling == null ? void 0 : parentSibling.id, hasHistoryOpen = !!((_b = parentSibling == null ? void 0 : parentSibling.params) != null && _b.rev), parentRefPath = useMemo(() => (params == null ? void 0 : params.parentRefPath) && fromString(params.parentRefPath) || null, [params == null ? void 0 : params.parentRefPath]), { t } = useTranslation(structureLocaleNamespace), referenceInfoObservable = useMemo(() => {
  ...
  }, [documentPreviewStore, parentId, parentRefPath]), referenceInfo = useObservable(referenceInfoObservable, { loading: !0 }), handleReloadReference = useCallback(() => {
    var _a2;
...
const preventDefault = (ev) => ev.preventDefault(), FormView = forwardRef(function(props, ref) {
  ...
  useEffect(() => {
    !focusedFirstDescendant && ready && !(formState != null && formState.focusPath.length) && formRef && (setFocusedFirstDescendant(!0), focusFirstDescendant(formRef));
...
const HistoryRestoreAction = ({ id, type, revision, onComplete }) => {
  ...
  } : null, [handleConfirm, isConfirmDialogOpen, onComplete, t]), isRevisionInitial = revision === "@initial";
  return revision === void 0 ? null : {
    label: t("action.restore.label"),
    color: "primary",
    onHandle: handle,
    title: t(
      isRevisionInitial ? "action.restore.disabled.cannot-restore-initial" : "action.restore.tooltip"
    ),
    icon: RestoreIcon,
    dialog,
...
function DocumentStatusBarActionsInner(props) {
  var _a, _b, _c;
  const { disabled, showMenu, states } = props, { __internal_tasks, schemaType, openPath } = useDocumentPane(), [firstActionState, ...menuActionStates] = states, [buttonElement, setButtonElement] = useState(null), isTreeArrayEditingEnabled = (_c = (_b = (_a = useSource().features) == null ? void 0 : _a.beta) == null ? void 0 : _b.treeArrayEditing) == null ? void 0 : _c.enabled, isTreeArrayEditingEnabledOpen = useMemo(() => isTreeArrayEditingEnabled ? shouldArrayDialogOpen(schemaType, openPath) : !1, [isTreeArrayEditingEnabled, openPath, schemaType]), tooltipContent = useMemo(() => !firstActionState || !firstActionState.title && !firstActionState.shortcut ? null : /* @__PURE__ */ jsxs(Flex, { style: { maxWidth: 300 }, align: "center", gap: 3, children: [
  ...
  return /* @__PURE__ */ jsxs(Flex, { align: "center", gap: 1, children: [
    ...
    firstActionState && /* @__PURE__ */ jsx(LayerProvider, { zOffset: 200, children: /* @__PURE__ */ jsx(Tooltip, { disabled: !tooltipContent, content: tooltipContent, placement: "top", children: /* @__PURE__ */ jsx(Stack, { children: /* @__PURE__ */ jsx(
      Button,
      {
        ...
        disabled: disabled || !!firstActionState.disabled || isTreeArrayEditingEnabledOpen,
...
const DocumentStatusBarActions = memo(function() {
  const { actions: allActions, connectionState, documentId, editState } = useDocumentPane(), actions = useMemo(
    () => (allActions != null ? allActions : []).filter((action) => !isRestoreAction(action)),
    [allActions]
  );
  return actions.length === 0 || !editState ? null : /* @__PURE__ */ jsx(
...
const DocumentPanelHeader = memo(
  forwardRef(function(_props, ref) {
    ...
    } = useDocumentPane(), { features } = useStructureTool(), { index, BackLink: BackLink2, hasGroupSiblings } = usePaneRouter(), { actions: fieldActions } = useFieldActions(), [referenceElement, setReferenceElement] = useState(null), actions = useMemo(
      () => (allActions != null ? allActions : []).filter((action) => !isRestoreAction(action)),
      [allActions]
    ), menuNodes = useMemo(
...
function usePreviewUrl(value) {
  const { resolveProductionUrl } = useSource().document, subject = useMemo(
  ...
  const resolvedUrlObservable = useMemo(() => subject.asObservable().pipe(
...
`, DocumentListPane = memo(function(props) {
  const { childItemId, isActive, pane: pane2, paneKey, sortOrder: sortOrderRaw, layout } = props, schema = useSchema(), { displayOptions, options } = pane2, { apiVersion, filter: filter2 } = options, params = useShallowUnique(options.params || EMPTY_RECORD), typeName = useMemo(() => {
    ...
    sortOrder
  }), handleQueryChange = useObservableEvent(
    (event$) => event$.pipe(
...
function DocumentSheetListSelect(props) {
  const { row, table } = props, { selectedAnchor, setSelectedAnchor } = table.options.meta || {}, handleOnClick = useCallback(
    (e) => {
      if (e.shiftKey && selectedAnchor !== null && selectedAnchor !== void 0) {
        const shiftClickIndex = row.index, lowerIndex = shiftClickIndex < selectedAnchor ? shiftClickIndex : selectedAnchor, upperIndex = shiftClickIndex < selectedAnchor ? selectedAnchor : shiftClickIndex, additionalSelectedRows = table.getRowModel().flatRows.slice(lowerIndex, upperIndex + 1).map(({ id }) => id), currentSelectedRows = table.getSelectedRowModel().rows.map(({ id }) => id);
        table.setRowSelection(
          () => [...additionalSelectedRows, ...currentSelectedRows].reduce(
            (nextSelectedRows, rowId) => ({ ...nextSelectedRows, [rowId]: !0 }),
            {}
          )
        );
...
function DocumentSheetListProvider({
  ...
  const [selectedAnchorCellDetails, setSelectedAnchorCellDetails] = useState(null), [selectedRangeCellIndexes, setSelectedRangeCellIndexes] = useState([]), clearAndSetFocusSelection = useCallback(
  ...
  ), resetFocusSelection = useCallback(
    () => clearAndSetFocusSelection(),
    [clearAndSetFocusSelection]
  ), changeSelectionColumn = useCallback(
    (direction) => {
  ...
  ), changeSelectionRange = useCallback(
    (direction) => {
    ...
    [selectedAnchorCellDetails]
  ), setSelectedAnchorCell = useCallback(
    (colId, rowIndex) => {
      clearAndSetFocusSelection({ colId, rowIndex, state: "selected" });
    },
    [clearAndSetFocusSelection]
  ), handleEscapePress = useCallback(() => {
  ...
  }, [clearAndSetFocusSelection, selectedAnchorCellDetails, selectedRangeCellIndexes.length]), handleUpDownKey = useCallback(
    (isShiftKey, key) => {
      if (!selectedAnchorCellDetails) return;
  ...
  ), handleAnchorKeydown = useCallback(
    (event) => {
      if (!selectedAnchorCellDetails) return;
...
function SheetListCellInner(props) {
  const { getValue, column, row, fieldType } = props, cellId = `cell-${column.id}-${row.index}`, [renderValue, setRenderValue] = useState(getValue()), [isDirty, setIsDirty] = useState(!1), inputRef = useRef(null), {
    focusAnchorCell,
    resetFocusSelection,
    setSelectedAnchorCell,
    getStateByCellId,
    ...
    setIsDirty(!0), setRenderValue(event.target.value);
  }, handleOnBlur = () => {
    isDirty && (patchDocument == null || patchDocument(row.id, column.id, renderValue), setIsDirty(!1)), resetFocusSelection();
  ...
  return fieldType.name === "boolean" ? /* @__PURE__ */ jsxs(
    Select,
    {
      ...inputProps,
      onChange: () => null,
...
const VISIBLE_COLUMN_LIMIT = 5, PreviewCell = (props) => {
  var _a, _b;
  const { documentPreviewStore, row, schemaType } = props, title = "Document title", previewStateObservable = useMemo(
...
function ColumnsControl({ table }) {
  const { t } = useTranslation(), isVisibleLimitReached = table.getVisibleLeafColumns().filter((col) => col.getCanHide()).length >= VISIBLE_COLUMN_LIMIT, setInitialColumns = useCallback(() => {
  ...
  return /* @__PURE__ */ jsx(
    MenuButton$1,
    {
      button: /* @__PURE__ */ jsx(Button$1, { mode: "bleed", text: t("sheet-list.edit-columns"), size: 1 }),
      id: "columns-control",
      menu: /* @__PURE__ */ jsx(Menu, { padding: 3, paddingTop: 4, style: { width: 240 }, children: /* @__PURE__ */ jsxs(Flex, { direction: "column", height: "fill", gap: 3, children: [
        /* @__PURE__ */ jsx(Text, { weight: "semibold", size: 1, children: t("sheet-list.select-fields") }),
        /* @__PURE__ */ jsx(Flex, { style: { flex: "1 1 auto", maxHeight: 320, overflowY: "scroll" }, children: /* @__PURE__ */ jsx(Stack, { children: table.getAllLeafColumns().filter((column) => column.getCanHide()).map((column) => {
          var _a;
...
function DocumentSheetListHeader(props) {
  ...
  return /* @__PURE__ */ jsx(
    HeaderTag,
    {
      ...
      children: /* @__PURE__ */ jsxs(Flex, { justify: "space-between", marginX: 2, align: "baseline", children: [
        /* @__PURE__ */ jsx(Tooltip, { delay: 500, content: headerTitle, children: /* @__PURE__ */ jsx(Box, { style: { boxSizing: "border-box" }, marginLeft: 3, marginRight: 3, children: headerTitle }) }),
        canShowHeaderMenu && /* @__PURE__ */ jsx(HoverMenu, { children: /* @__PURE__ */ jsx(
          MenuButton$1,
          {
            button: /* @__PURE__ */ jsx(
              Button,
...
function DocumentSheetListPaneInner({
  ...
  const renderRow = useCallback((row) => /* @__PURE__ */ jsx(
    Box,
    {
      ...
      children: row.getVisibleCells().map((cell) => /* @__PURE__ */ createElement(SheetListCell, { ...cell, key: row.original._id + cell.id }))
    },
  ...
  return /* @__PURE__ */ jsxs(PaneContainer$1, { direction: "column", paddingX: 3, "data-testid": "document-sheet-list-pane", children: [
    ...
    /* @__PURE__ */ jsx(TableContainer, { children: /* @__PURE__ */ jsx(DocumentSheetListProvider, { table, children: /* @__PURE__ */ jsxs(Table, { children: [
      /* @__PURE__ */ jsx("thead", { children: table.getHeaderGroups().map((headerGroup) => /* @__PURE__ */ jsx(Box, { as: "tr", children: headerGroup.headers.map((header) => /* @__PURE__ */ jsx(
