...
var toHtml = require('@portabletext/to-html');
...
const fieldLevelMerge = function (translatedFields, baseDoc, localeId) {
  ...
  const originPaths = mutator.extractWithPath("..".concat(baseLang), translatedFields);
  originPaths.forEach(match => {
    const origVal = mutator.extractWithPath(mutator.arrayToJSONMatchPath(match.path), baseDoc)[0].value;
    const translatedVal = mutator.extractWithPath(mutator.arrayToJSONMatchPath(match.path), translatedFields)[0].value;
    let valToPatch;
...
const preprocess = html => {
  const intermediateBlocks = blockTools.htmlToBlocks("<p>".concat(html, "</p>"), blockContentType);
  ...
  children.forEach(child => {
    ...
    try {
      if (((_a = child.tagName) == null ? void 0 : _a.toLowerCase()) === "span") {
        ...
        deserializedObject = blockTools.htmlToBlocks(child.outerHTML, blockContentType, {
...
exports.customBlockDeserializers = customBlockDeserializers;
